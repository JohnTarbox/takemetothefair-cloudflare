...
- **Main app Worker**: `meetmeatthefair-app` (OpenNext; former Pages project `takemetothefair`, retired 2026-06-10, project deleted 2026-09-24 — OPE-895)
...
| -------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Main app**   | `meetmeatthefair.com` (apex) | Next.js via `@opennextjs/cloudflare` (`main = .open-next/worker.js`), deployed as the **`meetmeatthefair-app` Worker** — was the Pages project `takemetothefair`, retired in the 2026-06-10 OpenNext cutover (`x-opennext: 1` on responses); the Pages project itself was deleted on 2026-09-24 (OPE-895), so there is no Pages rollback path. Serves every page/API route **including all `sitemap*.xml`** (Next.js route handlers in `src/app/sitemap*.xml/route.ts`). | `[[routes]] pattern = "meetmeatthefair.com/*"`, `zone_name = "meetmeatthefair.com"` in `wrangler.toml` — the Worker serves the apex directly (NOT Pages-native routing). |
