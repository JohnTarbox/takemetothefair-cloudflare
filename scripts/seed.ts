...
async function main() {
  ...
  db.insert(schema.events)
    .values({
      ...
      ticketPriceMinCents: 10 * 100,
      ticketPriceMaxCents: 25 * 100,
  ...
  db.insert(schema.events)
    .values({
      ...
      ticketPriceMinCents: 0 * 100,
      ticketPriceMaxCents: 5 * 100,
      featured: true,
      status: "APPROVED",
    })
    .run();
  ...
  db.insert(schema.events)
    .values({
      ...
      ticketPriceMinCents: 5 * 100,
      ticketPriceMaxCents: 10 * 100,
      featured: false,
      status: "APPROVED",
    })
    .run();
