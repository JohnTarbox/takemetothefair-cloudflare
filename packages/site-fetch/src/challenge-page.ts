...
const BODY_MARKERS: Array<{ re: RegExp; vendor: string }> = [
  ...
  // OPE-1154 — origin DENY pages, which Browser Rendering hands back as
  // ordinary HTML (its success shape carries no origin status). A 403 that
  // escalated to BR used to come back as `success: true` and be cited as
  // evidence: CloudFront's block page on a cheddarup.com registration link
  // (2026-09-22) and an "Access Denied" page behind share.google (2026-09-24).
  // Exact titles and CloudFront's own footer only — see the note above on
  // why a bare "access denied" phrase is not a marker.
  { re: /<title>\s*ERROR: The request could not be satisfied\s*<\/title>/i, vendor: "cloudfront" },
  { re: /Generated by cloudfront \(CloudFront\)/i, vendor: "cloudfront" },
  { re: /<title>\s*Access Denied\s*<\/title>/i, vendor: "access-denied" },
  { re: /<title>\s*403 Forbidden\s*<\/title>/i, vendor: "http-403" },
];
...
/**
 * OPE-1154 — the same classification for a page we only kept as TITLE + TEXT.
 *
 * Citation snapshots store the fetched page's `<title>` and extracted text,
 * not its HTML, so the title-anchored markers above cannot see them directly.
 * Rebuilding a minimal document puts the title back where those markers look;
 * body-text markers (CloudFront's footer) match the text as-is.
 */
export function detectBlockedSnapshot(
  title: string | null | undefined,
  text: string | null | undefined
): ChallengeVerdict {
  return detectChallengePage(`<title>${title ?? ""}</title>\n${text ?? ""}`, null);
}
