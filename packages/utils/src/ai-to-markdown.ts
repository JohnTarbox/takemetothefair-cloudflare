/**
 * Retrying wrapper around `env.AI.toMarkdown` (OCR / document→markdown).
 *
 * Promoted to the workspace package by OPE-297 so the main app's image-intake
 * lane and the MCP Worker's attachment OCR share ONE implementation. Both have
 * their own `AI` binding; only the retry semantics below are worth agreeing on,
 * and a second copy would drift exactly the way OPE-285 did.
 *
 * OPE-189 — original context.
 *
