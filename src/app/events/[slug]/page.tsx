...
import { STATUS_BADGE_VARIANTS } from "@/lib/vendor-status";
...
// `asOccurrence` is a 2nd positional arg (not a prop): Next renders the default
// page export with a single props object, so it defaults to false for the bare
// /events/[slug] route, while the /year occurrence route calls this as a plain
// function with `true`. Putting it on Props would fail Next's PageProps check. (K46)
export default async function EventDetailPage({ params }: Props, asOccurrence = false) {
  const { slug } = await params;
