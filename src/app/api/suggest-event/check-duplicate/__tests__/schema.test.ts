...
// findDuplicate (whose input treats null/undefined as "missing"). The schema
// normalizes null -> undefined so downstream consumers see a null-free value.
describe("checkDuplicateSchema null tolerance", () => {
  it("accepts an explicit null on a venue field and normalizes it to undefined", () => {
    ...
    if (r.success) expect(r.data.venueCity).toBeUndefined();
  });
