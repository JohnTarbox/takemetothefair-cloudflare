...
async function maxFor(query: Promise<Array<{ value: Date | null }>>): Promise<Date | null> {
  try {
    ...
    if (!v) return null;
    // Drizzle returns Date for timestamp columns; raw SQL returns seconds-
    // epoch number. The vendors path below uses sql<number> and converts,
    // so by the time we get here the value is always Date or null.
