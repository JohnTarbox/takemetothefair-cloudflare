...
function seedEvent(id: string, name: string, startIso: string, sourceUrl: string | null) {
  raw
    .prepare(
      `INSERT INTO events (id, slug, name, start_date, status, source_url)
       VALUES (?,?,?,?,?,?)`
    )
    .run(id, id, name, epoch(startIso), "APPROVED", sourceUrl);
}
...
describe("the guard still catches genuine duplicates", () => {
  ...
  it("blocks within the ±7d window, not only on an exact date", async () => {
    // A source that reports the fair's second day must still match the first.
