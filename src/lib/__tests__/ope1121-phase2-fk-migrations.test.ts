/**
 * OPE-1121 Phase 2 — the 13 child-table rebuilds that add foreign keys.
 *
...
const FILES = readdirSync(DIR)
  .filter((f) => /^03(0[89]|1\d|20)_ope1121_fk_.+\.sql$/.test(f))
  .sort();

const tableOf = (f: string) => f.replace(/^\d{4}_ope1121_fk_/, "").replace(/\.sql$/, "");

describe("OPE-1121 Phase 2 migrations", () => {
  it("there are exactly 13, ledger first and delivery events after it", () => {
