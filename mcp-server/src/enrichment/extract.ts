...
const PLATFORM_PLACEHOLDER_HANDLES = new Set([
  ...
  "wordpressdotcom",
  "webflow",
  // OPE-963 — named by the promoter-lane specimen list.
  "squareup",
  "wixpress",
  "bigcommerce",
  "duda",
...
/**
 * The account handle of a social URL, normalized to [a-z0-9]: the last path
 * segment (`facebook.com/wix`, `youtube.com/c/Wix`, `instagram.com/squarespace/`).
 * Null when the URL does not parse or has no path.
 */
export function socialHandleOf(url: string): string | null {
  let path: string;
  try {
  ...
  return segments[segments.length - 1].toLowerCase().replace(/[^a-z0-9]/g, "") || null;
}

export function isPlatformPlaceholderHandle(url: string): boolean {
  const handle = socialHandleOf(url);
  return handle !== null && PLATFORM_PLACEHOLDER_HANDLES.has(handle);
}

/**
 * OPE-963 — the site builder a page says it was made with, from its own
 * `<meta name="generator">` ("Squarespace", "Wix.com Website Builder",
 * "WordPress 6.4"). The first word, normalized; empty when there is none.
 *
 * Catches platform self-reference that the fixed handle list does not name: a
 * page generated by X linking to X's own accounts.
 */
export function detectSitePlatforms(html: string): string[] {
  const out = new Set<string>();
  const re =
    /<meta[^>]*name=["']generator["'][^>]*content=["']([^"']+)["'][^>]*>|<meta[^>]*content=["']([^"']+)["'][^>]*name=["']generator["'][^>]*>/gi;
  for (const m of html.matchAll(re)) {
    const first = (m[1] ?? m[2] ?? "").toLowerCase().match(/[a-z]+/);
    if (first && first[0].length >= 3) out.add(first[0]);
  }
  return [...out];
}
