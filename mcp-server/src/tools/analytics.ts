...
export function registerAnalyticsTools(server: McpServer, auth: AuthContext, env?: Env) {
  ...
  server.tool(
    ...
    [
      ...
      "",
      "READ `unevaluated` BEFORE CONCLUDING THE QUEUE IS THE WORKLIST (OPE-547).",
      "`producerClass` is scoped to PAST + producer-class + OCCURRED, and `queue`",
      "counts only the four queue statuses — so a row with NO status at all was",
      "invisible to both, and `producerClass.unevaluated` read 0 while 651",
      "research targets carried none. A drain that sets terminal verdicts on",
      "those rows moves `needsResearchTotal` by ZERO; that is the metric being",
      "narrow, not the work not counting.",
      "`unevaluated.past` is the real gap (the event is over and nothing was ever",
      "decided). `unevaluated.upcoming` is expected and fine — there is nothing",
      "to research yet. `unevaluated.withRosterGradeLinks` is the sharp one:",
      "rows already holding >= rosterGradeLinkThreshold non-sponsor vendor links",
      "that no status records, i.e. finished work with no receipt.",
      "",
      "PRODUCER-CLASS MEMBERSHIP KEYS ON `categories`, NOT SCALE (OPE-713).",
      "`producerClassDefinition` returns the exact predicate — read it BEFORE a",
      "pass, not after, so you know whether your target will register. A drain",
      "on 2026-08-31 added 530 links and moved `coveragePct` 0.4pp because the",
      "two largest rosters it completed (93 and 81 exhibitors) carried EMPTY",
      "categories, and two more carried Festival/Community Event. Scale was",
      "suspected and is not in the predicate at all.",
      "`producerClassExcluded` reports those rows and splits them by CAUSE,",
      "because the remedies are opposite: `emptyCategories` is a DATA gap that",
      "no metric change can reach (categorise the event and it joins the",
      "denominator), while the rest are the definition working as written.",
      "`allPastOccurred` is the same frame with the category filter dropped —",
      "the number a drain can actually move. `producerClass` is deliberately",
      "unchanged so a published series does not silently restate its own history.",
  ...
  server.tool(
    ...
    [
      ...
      "disagreements (rejected), agreementPct, sampleSize, plus the HUMAN-only figures",
      "(humanApproved, humanRejected, humanSampleSize, humanAgreementPct — auto_merged",
      "excluded, OPE-963) and a `promotable` flag gated on the human figure",
      "(≥95% over ≥20 human decisions). agreementPct counts a rule's own auto-applies",
      "as agreement, so do not use it to judge a rule. Use this to decide which extraction",
