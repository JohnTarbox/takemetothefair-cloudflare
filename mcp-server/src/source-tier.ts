 ...
 *
 * Mirror of src/lib/source-tier.ts in the main app. The two files MUST be
 * kept in sync — Worker isolates can't share src/ imports across the
 * mcp-server / main-app boundary, so we duplicate the pure logic here.
 * Same pattern as decodeHtmlEntities (src/lib/utils.ts ⇔ mcp-server/src/
 * helpers.ts) per feedback_mcp_input_decode.
 *
