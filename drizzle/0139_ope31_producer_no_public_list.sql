...
-- This adds promoters.vendor_roster_publishes_lists and, for the confirmed
-- producer, (a) sets the flag = 0 (false) and (b) back-fills their
...
--   * Producer match is by company_name. Names VERIFIED against live prod
--     `promoters` on 2026-06-30 (OPE-31 review): 'Great New England Craft &
--     Artisan Shows' matched exactly (id 3cbf0d4a-…). A second candidate,
--     'Hartford Quechee Chamber', had NO prod match (searched Quechee/Hartford/
--     Chamber) and was DROPPED from this migration — it can be flagged later via
--     `update_promoter` once/if that producer exists in the DB. The occurred-sweep
--     rails + `update_promoter` still let an operator flag any producer
--     post-deploy, so this migration is not the only path to the flag.

ALTER TABLE promoters ADD COLUMN vendor_roster_publishes_lists integer;

-- (a) Flag the confirmed producer (verified producer-wide "no public roster").
UPDATE promoters
SET vendor_roster_publishes_lists = 0, updated_at = unixepoch()
WHERE company_name = 'Great New England Craft & Artisan Shows';
